...
class MoEModelConfig:
    ...
    # Z-experts specific parameters
    num_zero_experts: int = 2  # Number of zero-computation experts to add
...
class ZeroExpert(nn.Module):
    """Zero-computation expert that simply passes through the input unchanged"""
    def __init__(self):
        super().__init__()

    def forward(self, x):
        """Return input unchanged - zero computation"""
        return x
...
class MixtureOfExperts(nn.Module):
    """Mixture of Experts layer with top-k routing and Z-experts"""
    def __init__(
        self,
        d_model: int,
        d_ff: int,
        num_experts: int = 8,
        num_zero_experts: int = 2,
        top_k: int = 2,
        dropout: float = 0.1,
        load_balancing_weight: float = 0.01
    ):
        ...
        # Create experts: standard experts + zero experts
        ...
        # Create router (routes to all experts including zero experts)
        self.router = TopKRouter(d_model, self.total_experts, top_k)

        # Track zero expert usage for analysis
        self.zero_expert_usage = []
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        # Track zero expert usage
        zero_expert_mask = expert_indices >= self.num_experts  # Zero experts are at indices [num_experts, total_experts)
        total_tokens = batch_size * seq_len
        zero_tokens = zero_expert_mask.sum().item()
        zero_usage_ratio = zero_tokens / (total_tokens * self.top_k)
        self.zero_expert_usage.append(zero_usage_ratio)
    ...
    def get_zero_expert_stats(self):
        """Return statistics about zero expert usage"""
        if not self.zero_expert_usage:
            return {"avg_usage": 0.0, "total_measurements": 0}

        return {
            "avg_usage": np.mean(self.zero_expert_usage),
            "total_measurements": len(self.zero_expert_usage)
        }

class MoETransformerBlock(nn.Module):
    """Transformer block with MoE and Z-experts"""
    def __init__(
        self,
        d_model: int,
        n_heads: int,
...
class MoEMinimalLLM(nn.Module):
    """Minimal LLM with Mixture of Experts and Z-experts"""
    def __init__(self, config: MoEModelConfig):
        ...
        # Transformer blocks with MoE and Z-experts
    ...
    def get_zero_expert_stats(self):
        """Get aggregated zero expert usage statistics across all layers"""
        layer_stats = []
        for block in self.transformer_blocks:
            stats = block.feed_forward.get_zero_expert_stats()
        ...
        avg_usage = np.mean([s["avg_usage"] for s in layer_stats])
        total_measurements = sum([s["total_measurements"] for s in layer_stats])
...
def train_moe_model(config: MoEModelConfig, train_loader: DataLoader, val_loader: DataLoader, experiment_name: str = "z_experts"):
    """Train the MoE model with Z-experts"""
    print(f"\n🚀 Training MoE model with {config.num_experts} experts + {config.num_zero_experts} Z-experts (top-{config.expert_top_k})")
    ...
    # Track zero expert usage over time
    zero_usage_history = []
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            if step % 100 == 0:
                with torch.no_grad():
                    ...
                    # Get zero expert usage stats
                    zero_stats = model.get_zero_expert_stats()
                ...
                postfix_dict = {
                    ...
                    'acc': f'{accuracy:.3f}',
                    'ppl': f'{perplexity:.1f}',
                    'z_usage': f'{zero_stats["avg_usage"]:.3f}'
                }
                ...
                # Track zero usage over time
                zero_usage_history.append({
                    'step': step,
                    'zero_usage': zero_stats["avg_usage"],
                    'loss': current_loss,
                    'accuracy': accuracy
                })
            ...
            if step % config.eval_every == 0 and step > 0:
                ...
                print(f"\nStep {step}: Val Loss: {eval_metrics['val_loss']:.4f}, "
                      ...
                      f"Z-Usage: {zero_stats['avg_usage']:.3f}")

            # Milestone evaluations
            if step in getattr(config, 'log_milestones', ()):
                eval_metrics = evaluate_model(model, val_loader, config)
                zero_stats = model.get_zero_expert_stats()
                print(f"\n🧪 Milestone {step}: Val Loss: {eval_metrics['val_loss']:.4f}, Z-Usage: {zero_stats['avg_usage']:.3f}")
    ...
    final_zero_stats = model.get_zero_expert_stats()

    print(f"\n📊 Final Results for {experiment_name}:")
    print(f"   Val Loss: {final_eval['val_loss']:.4f}")
    print(f"   Val Accuracy: {final_eval['val_accuracy']:.4f}")
    print(f"   Val Perplexity: {final_eval['val_perplexity']:.2f}")
    print(f"   Z-Expert Usage: {final_zero_stats['avg_usage']:.3f}")

    # Return model and results including zero expert statistics
    results = {
        **final_eval,
        'zero_expert_usage': final_zero_stats['avg_usage'],
        'zero_usage_history': zero_usage_history,
        'experiment_name': experiment_name
    }

    return model, results

# Experiment configurations for Z-experts
def run_z_expert_experiments():
    """Run experiments comparing different Z-expert configurations"""

    print(f"🔬 Z-EXPERT EXPERIMENTS")
    print(f"{'='*60}")
    ...
    experiments = [
        {
            'name': 'baseline_8experts',
            'num_experts': 8,
            'num_zero_experts': 0,
            'description': 'Standard MoE with 8 experts, no Z-experts'
        },
        {
            'name': 'z_experts_8plus2',
            'num_experts': 8,
            'num_zero_experts': 2,
            'description': '8 standard experts + 2 Z-experts'
        },
        {
            'name': 'z_experts_6plus4',
            'num_experts': 6,
            'num_zero_experts': 4,
            'description': '6 standard experts + 4 Z-experts (50% Z-experts)'
        },
        {
            'name': 'z_experts_4plus8',
            'num_experts': 4,
            'num_zero_experts': 8,
            'description': '4 standard experts + 8 Z-experts (double Z-experts)'
        }
    ]
    ...
    for exp in experiments:
        ...
        # Configure model for this experiment
        config = MoEModelConfig(
            ...
            d_ff=1536,
            batch_size=24,
            max_steps=1500,  # Shorter for experiments
            eval_every=300,   # More frequent evaluation
            vocab_size=vocab_size,

            # MoE specific
            num_experts=exp['num_experts'],
            num_zero_experts=exp['num_zero_experts'],
            expert_top_k=2,
        ...
        print(f"   MoE: {config.num_experts} experts + {config.num_zero_experts} Z-experts, top-{config.expert_top_k} routing")
        print(f"   Training: {config.max_steps} steps, batch size {config.batch_size}")

        # Train model
        start_time = time.time()
        model, exp_results = train_moe_model(config, train_loader, val_loader, exp['name'])
        total_time = time.time() - start_time

        exp_results['training_time'] = total_time
        exp_results['experiment_config'] = exp
        results.append(exp_results)

        print(f"\n🎯 {exp['name']} Results:")
        print(f"⏱️ Training time: {total_time/60:.1f} minutes")
        print(f"🏆 Final Results:")
        print(f"   Validation Loss: {exp_results['val_loss']:.4f}")
        print(f"   Validation Accuracy: {exp_results['val_accuracy']:.4f}")
        print(f"   Validation Perplexity: {exp_results['val_perplexity']:.2f}")
        print(f"   Z-Expert Usage: {exp_results.get('zero_expert_usage', 0):.3f}")
        print(f"{'='*60}")

    # Summary comparison
    print(f"\n📊 EXPERIMENT SUMMARY")
    print(f"{'='*60}")
    print(f"{'Experiment':<20} {'Loss':<8} {'Acc':<8} {'PPL':<8} {'Z-Usage':<8} {'Time(min)':<10}")
    print(f"{'-'*60}")
    ...
    for result in results:
        ...
        ppl = result['val_perplexity']
        z_usage = result.get('zero_expert_usage', 0)
        train_time = result['training_time'] / 60

        print(f"{exp_name:<20} {loss:<8.4f} {acc:<8.4f} {ppl:<8.2f} {z_usage:<8.3f} {train_time:<10.1f}")

    return results

if __name__ == "__main__":
    # Run Z-expert experiments
    experiment_results = run_z_expert_experiments()

    # Save results for analysis
    results_file = "z_expert_experiment_results.pkl"
